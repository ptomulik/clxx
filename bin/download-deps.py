...
def scons_version_string(v):
    if v not in scons_versions:
        raise argparse.ArgumentTypeError("ill-formed or unsupported SCons version %r" % v)
    return v
...
parser.add_argument(
        ...
        default=default_scons_arguments_version,
        metavar='VER',
        help='version of scons-arguments to be downloaded'
        )
...
def dload_scons_arguments(**kw):
    ...
    try: destdir = kw['destdir']
    except KeyError: destdir = os.path.join(topsrcdir, 'site_scons', 'SConsArguments')
    ...
    if os.path.exists(destdir):
        warn("'%s' already exists, skipping scons_arguments download!" % destdir, **kw)
        return 2

    try: ver = kw['scons_arguments_ver']
    except KeyError: ver = None
    if ver is None:
        ver = default_scons_arguments_version
    ...
    src = os.path.join(tmpdir, 'SConsArguments')
    dst = destdir
    info("copytree '%s' -> '%s'" % (src, dst), **kw)
    shutil.copytree(src, dst)
